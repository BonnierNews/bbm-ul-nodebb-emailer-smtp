...
Emailer.send = function(data, callback) {
    ...
    var smtpConfig = {
        ...
        secure: settings['emailer:local:secure'] === 'off' ? false : true,
        requireTLS: settings['emailer:local:tls'] === 'off' ? false : true
    };

    // Authentication
    if(settings['emailer:local:enableauth'] === 'on') {
        smtpConfig.auth = {
    ...
    if(settings['emailer:local:insecuretls'] === 'on') {
        smtpConfig.tls = {
